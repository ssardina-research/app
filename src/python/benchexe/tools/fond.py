...
class Tool(BaseTool2):
    ...
    def executable(self, tool_locator):
        return tool_locator.find_executable("pp-fond.py")
    ...
    def determine_result(self, run):
        ...
        for line in run.output:
            # for PRP
            if "Dumping the policy and fsaps" in line: 
                status = result.RESULT_TRUE_PROP
            # for paladinus
            elif "Result: Policy successfully found" in line:
                status = result.RESULT_TRUE_PROP
    ...
    def _get_translation_time(self, output):
        translation_time = -1
        for _l in output:
            if "Translation of APP To FOND Time" in _l:
    ...
    def _get_policy_size(self, output):
        for _l in output:
            if "Found a policy of size:" in _l:
