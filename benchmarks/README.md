...
> [!NOTE]
> The APP-PDDL format is a much more improved and clean-up version of the original set of APP problems done for the PP-LPG solver in AIJ'16 paper. There, APP were specified with a collection of files for easier handling by the PP-LPG solver. Such format is not used here and the meta-solvers in this repo will translate APP-PDDL specification to formats relevant for them, including the one required for the PP-LPG solver.
...
2. The second folder denotes the on shape of the APPs (e.g., `RND/` for random APPs).
...
This is a benchmark consisting of non-deterministic domain from Blocksworld. The APP program graphs are the same from Blocksworld domain from AIJ'16, except for the domain. Introduced for the ICAPS'25 paper which is the only solver that can handle non-deterministic underlying domains.
