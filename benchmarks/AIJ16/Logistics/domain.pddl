(define (domain logistics-strips)
  ...
  (:types
    ...
    location city airport mobject - object
  )
  (:predicates
    (at ?mobject - mobject ?location - location)
  ...
  (:action DRIVE-TRUCK
    ...
    :precondition (and (not (at ?truck ?loc-to)) (at ?truck ?loc-from) (in-city ?loc-from ?city)(in-city ?loc-to ?city))
    :effect (and (not (at ?truck ?loc-from)) (at ?truck ?loc-to))
  )

  (:action FLY-AIRPLANE
    :parameters (?airplane - airplane ?loc-from - location ?loc-to - location)
    :precondition (and (not (at ?airplane ?loc-to))
